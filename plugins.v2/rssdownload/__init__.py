...
class RssDownload(_PluginBase):
    # 插件名称
    plugin_name = "RSS 直接下载"
    # 插件描述
    plugin_desc = "定时刷新 RSS 报文，匹配规则后直接下载，不检查媒体库是否已存在。"
    # 插件图标
    plugin_icon = "rss.png"
    # 插件版本
    ...
    def init_plugin(self, config: dict = None):
        ...
        if self._onlyonce:
            ...
            logger.info("RSS 直接下载服务启动，立即运行一次")
                    ...
                    "content": [
                        {
                            ...
                            "events": {
                                "click": {
                                    "api": "plugin/RssDownload/delete_history",
                                    "method": "get",
                                    "params": {
                                        "key": title,
        ...
        for url in self._address.split("\n"):
            ...
            for result in results:
                try:
                    ...
                    logger.error(
                        f"刷新 RSS 数据出错：{str(err)} - {traceback.format_exc()}"
                    )
    ...
    def __log_and_notify_error(self, message):
        ...
        self.systemmessage.put(message, title="RSS 直接下载")
    ...
    def __validate_and_fix_config(self, config: dict = None) -> bool:
        ...
        if size_range and not self.__is_number_or_range(str(size_range)):
            self.__log_and_notify_error(
                f"RSS 直接下载出错，种子大小设置错误：{size_range}"
            )
